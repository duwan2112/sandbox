...
export default function Landing({id, query, dataB, websiteB}) {
            ...
            <BannerDesktop background="page-background-0.png">
              ...
              >
                <p className={`web-text mr-3 ${!activePolitic && "off"}`}>
                  ...
                  <Link
                    href={{
                      pathname: `/p/${website.url}/${data.subareaName}/politicy/cookies`,
                      query: {
...
Landing.getInitialProps = async (ctx) => {
  const res = await fetch(
    `${process.env.SERVER_API_URL}/users/getSubareaP/${ctx.query.subarea}`
  );
  const {ok, data: result} = await res.json();

  const res2 = await fetch(
    `${process.env.SERVER_API_URL}/users/getwebsite/${ctx.query.url}`
  );
  ...
  return {query: ctx.query, dataB: result, websiteB: result2};
};
