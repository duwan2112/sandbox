...
const Legal = ({data}) => {
  ...
  useEffect(() => {
    fetch(`${process.env.SERVER_API_URL}/users/getwebsite/${data.url}`)
      ...
      .then((result) =>
        result.data.politics ? setLawyer(result.data.politics.privacy) : null
      );
  }, []);
  ...
  return (
    <>
      <Head>
        <title>Politica de privacidad | Abogados</title>
        <link
          rel="icon"
          type="image/png"
          sizes="16x16"
      ...
      <Container>
        <div className="arrow-politics mt-5">
          <Link
            href={`${
              data.idSub
                ? `/landing-preview/${data.nameSub}?id=${data.idSub}`
                : `/preview/[url]`
            }`}
            as={`${
              data.idSub
        ...
        {lawyer ? (
          ...
          <p className="faq__text">
            Aún no se han definido las políticas de privacidad
          </p>
        )}
      </Container>
    </>
